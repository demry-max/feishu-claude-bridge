...
npm install @larksuiteoapi/node-sdk dotenv qrcode-terminal cross-spawn cron-parser
```
...
```bash
mkdir -p ~/feishu-claude-bridge/workspace/memory/journal ~/feishu-claude-bridge/workspace/skills
```
...
被问到「你用什么模型」「什么思考档位」时，**只依据上面 runtime.md 的内容回答**。你无法从自身推断真实模型——凭记忆猜测必然出错。
